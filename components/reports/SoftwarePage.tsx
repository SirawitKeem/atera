      ...
      <div className="page-content space-y-4 flex-1 flex flex-col justify-start overflow-hidden mt-3">
        ...
        <div className="grid grid-cols-4 gap-3 select-none">
          ...
          <div className="bg-white border border-slate-100 rounded-xl p-3 flex flex-col justify-between shadow-xs h-[74px]">
            <div className="flex items-center justify-between">
              <span className="text-[7.5px] font-extrabold text-slate-400 uppercase tracking-wider block">Total Available Updates</span>
              <Download className="h-3.5 w-3.5 text-indigo-500" />
            </div>
            <div>
              <h4 className="text-base font-black text-indigo-600 leading-none">{totalAvailableUpdates} Updates</h4>
              <p className="text-[6.5px] text-slate-400 font-bold uppercase mt-1">อัปเดตทั้งหมดที่รอการติดตั้ง</p>
            </div>
          </div>
          ...
          <div className="bg-white border border-slate-100 rounded-xl p-3 flex flex-col justify-between shadow-xs h-[74px]">
            ...
            <div>
              <h4 className="text-base font-black text-blue-600 leading-none">{securityUpdates} Updates</h4>
              <p className="text-[6.5px] text-slate-400 font-bold uppercase mt-1">อัปเดตด้านความปลอดภัย</p>
            </div>
          </div>
          ...
          <div className="bg-white border border-slate-100 rounded-xl p-3 flex flex-col justify-between shadow-xs h-[74px]">
            ...
            <div>
              <h4 className="text-base font-black text-blue-500 leading-none">{definitionUpdates} Updates</h4>
              <p className="text-[6.5px] text-slate-400 font-bold uppercase mt-1">อัปเดตฐานข้อมูลไวรัส</p>
            </div>
          </div>
          ...
          <div className="bg-white border border-slate-100 rounded-xl p-3 flex flex-col justify-between shadow-xs h-[74px]">
            <div className="flex items-center justify-between">
              <span className="text-[7.5px] font-extrabold text-slate-400 uppercase tracking-wider block">Devices Needing Updates</span>
              <Monitor className="h-3.5 w-3.5 text-indigo-500" />
            </div>
            <div>
              <h4 className="text-base font-black text-indigo-600 leading-none">{devicesNeedingUpdates} Devices</h4>
              <p className="text-[6.5px] text-slate-400 font-bold uppercase mt-1">อุปกรณ์ที่ต้องอัปเดต</p>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-1.5 flex-1 flex flex-col overflow-hidden mt-2">
          <h3 className="text-[9px] font-black text-slate-800 uppercase tracking-wider flex items-center gap-1.5 select-none">
            <Download className="h-3.5 w-3.5 text-indigo-500" /> รายการซอฟต์แวร์ที่ต้องอัปเดตแยกตามอุปกรณ์ (Software Updates by Device)
          </h3>
            ...
            <div className="overflow-y-auto flex-1">
              <table className="min-w-full divide-y divide-slate-100 text-[10px] text-left">
                <thead className="bg-[#0f4c81] text-white font-bold uppercase tracking-wider text-[7.5px] sticky top-0 z-10">
                  <tr>
                    <th className="px-4 py-2 w-[20%]">DEVICE NAME</th>
                    <th className="px-4 py-2 w-[15%]">OS</th>
                    <th className="px-4 py-2 w-[15%]">CLASSIFICATION</th>
                    <th className="px-4 py-2 w-[15%]">KB ID</th>
                    <th className="px-4 py-2 w-[25%]">PATCH NAME</th>
                    ...
                    <React.Fragment key={agent.deviceGuid || agentIdx}>
                      ...
                      <tr className="bg-slate-100/80">
                        <td colSpan={6} className="px-4 py-1.5 font-extrabold text-slate-800 text-[9px] border-b border-slate-200">
                          {agent.agentName} <span className="text-slate-500 font-medium">({agent.availablePatches.length} updates)</span>
                        </td>
                      </tr>
                      {/* Patches Rows */}
                  ...
                  {agentsWithUpdates.length === 0 && (
                    <tr>
                      <td colSpan={6} className="px-4 py-8 text-center text-slate-400 font-medium">
                        ...
                        อุปกรณ์ทั้งหมดได้รับการอัปเดตเป็นรุ่นล่าสุดแล้ว
                      </td>
                    </tr>
                  )}
                </tbody>
